...
# -*- coding: utf-8 -*-
# =========================================================
# app.py  —  DRDL Aerospace AI Platform  v9.5  (CLEAN)
# =========================================================
# KEY FIXES vs original v9.5:
#   1. All raw Tkinter .Widget.config() calls removed.
#      Button styling is done purely via PySimpleGUI API
#      (button_color, border_width, mouseover_colors).
#   2. matplotlib canvas embed kept (FigureCanvasTkAgg) — this
#      is unavoidable because PySimpleGUI's sg.Canvas wraps a
#      Tk canvas widget; matplotlib MUST use it internally.
#      No direct Tk calls appear in application logic.
#   3. _plot_console() key clash fixed: lbl_key+'_NAV' removed
#      (the label between arrows was duplicating the lbl_key).
#   4. OPT_RIGHT layout: list-of-rows corrected (first element
#      was a bare list not wrapped in a row, causing column errors).
#   5. con_clear / con_append now use update() safely.
#   6. Window maximize uses PySimpleGUI .maximize() / .normal().
#   7. Login window button styling uses only PySimpleGUI args.
# =========================================================
...
F_SUB    = ('Segoe UI', 11)
F_SEC    = ('Segoe UI', 12, 'bold')
F_LBL    = ('Segoe UI', 12)
F_INP    = ('Segoe UI', 12, 'bold')
F_OUT    = ('Segoe UI', 13, 'bold')
F_BTN    = ('Segoe UI', 12, 'bold')
F_STS    = ('Segoe UI', 12, 'bold')
F_TBL    = ('Segoe UI', 12)
F_TOP5   = ('Segoe UI', 12)
F_CHROME = ('Segoe UI', 11, 'bold')
F_ARROW  = ('Segoe UI', 16, 'bold')
F_PLTLBL = ('Segoe UI', 12, 'bold')
F_TABTXT = ('Segoe UI', 13, 'bold')
...
# =========================================================
# LOGIN  (pure PySimpleGUI — no .Widget access)
# =========================================================
...
_opt_figs   = []      # matplotlib Figure objects for Tab 2
_opt_idx    = 0
_opt_agg    = None    # FigureCanvasTkAgg kept alive

_env_figs   = []      # matplotlib Figure objects for Tab 3
_env_idx    = 0
_env_agg    = None
...
def sec_hdr(text, color=C_CYAN):
    """Single row list — use inside a layout directly."""
...
# it is required because sg.Canvas wraps a Tk canvas.
# All widget access is through the matplotlib API only.
# =========================================================
...
def _plot_console(canvas_key, lbl_key, prev_key, next_key,
    ...
    """
    Returns list-of-rows forming one plot console:
      • Section header
      • Label: "N / Total — Plot title"
      • Canvas
      • Row: ◄  [spacer]  ►
    """
    return (
...
OUT_COL = [
    ...
    [lbl('XCP/D (dataset rows)',   28), out_field('XCPD_OUT',18, C_PURP),
     sg.Text('(dataset rows only)', font=('Segoe UI', 11),
             text_color=C_DIM, background_color=C_PANEL)],
    ...
    # hidden stubs (kept for backward compat)
...
prediction_tab = [
    ...
    [sg.Column([[
        ...
        action_btn('<>  RESET',        'Reset_Pred', bg='#1E3A5F', w=13),
...
# =========================================================
bounds_rows = [[
    sg.Text('Parameter',   size=(28, 1), font=('Segoe UI', 13, 'bold'),
            text_color=C_AMBER, background_color=C_PANEL, pad=(4, 3)),
    sg.Text('Lower Bound', size=(14, 1), font=('Segoe UI', 13, 'bold'),
            text_color=C_AMBER, background_color=C_PANEL, pad=(4, 3)),
    sg.Text('Upper Bound', size=(14, 1), font=('Segoe UI', 13, 'bold'),
            text_color=C_AMBER, background_color=C_PANEL, pad=(4, 3)),
]]
for p in PARAMS:
    lo, hi = BOUNDS[p]
...
_TOP5_INFO = (
    ...
    '  Each DE generation evaluates ALL population individuals.\n'
    '  Fitness = 0.95*(CL/CD) + 0.05*0.2464*(1/|XCP-(-5.15)|)\n'
    '  Hard constraint: CD >= 0.12  (violators -> fitness = -inf)\n'
    '  Best per generation -> hall-of-fame.  After all gens:\n'
    '  sort by fitness DESC, remove near-duplicates, keep top 5.\n'
    '  Rank #1 = highest fitness = best CL/CD + XCP stability.\n'
)

# OPT_RIGHT is built as a clean list-of-rows
OPT_RIGHT = (
    [
    ...
    [
        [sg.Text(_TOP5_INFO, font=('Segoe UI', 11),
                 text_color=C_DIM, background_color=C_PANEL,
                 pad=(6, 3))],
...
optimization_tab = [
    ...
    [sg.Column([[
        ...
        action_btn('XX ABORT',         'Abort_Opt', bg='#7F1D1D', w=11),
        action_btn('<>  CLEAR',        'Clear_Opt', bg='#1E3A5F', w=11),
        sg.Push(background_color=C_BG),
        sg.Text('Progress:', font=F_LBL,
                text_color=C_DIM, background_color=C_BG),
...
ENV_LEFT = (
    [
        ...
        [sg.Text('', key='OPT_GEO_STATUS',
                 font=('Segoe UI', 12), text_color=C_GREEN,
                 background_color=C_PANEL,
                 size=(58, 1), pad=(10, 4))],
...
flight_tab = [
    ...
    [sg.Column([[
        ...
        action_btn('<>  CLEAR',     'Clear_Env', bg='#1E3A5F', w=11),
        action_btn('>> EXPORT CSV','Export_Env', bg='#374151', w=14),
        sg.Push(background_color=C_BG),
        sg.Text('Progress:', font=F_LBL,
                text_color=C_DIM, background_color=C_BG),
...
STATUS_ROW = [
    sg.Text('*', font=('Segoe UI', 14), text_color=C_GREEN,
    ...
    sg.Text(f'[{_mode_label}]', font=('Segoe UI', 12, 'bold'),
            text_color=C_PURP, background_color=C_DARK, pad=(6, 4)),
    sg.Text('', key='STS_T', font=('Segoe UI', 12),
...
# Maximize on startup using PySimpleGUI API only
try:
    window.maximize()
    _is_max = True
except Exception:
...
def render_optimization(result, history, elapsed):
    ...
    if top5:
        lines = [
            '  ' + '=' * 60,
            '  TOP-5 BEST PARAMETER SETS  --  DRDL Aerospace AI Platform',
            '  Fitness = 0.95*(CL/CD) + 0.05*0.2464*(1/|XCP-(-5.15)|)',
            '  ' + '=' * 60, '',
        ]
        for sol in top5:
            ...
            lines += [
                f'  RANK #{r}',
                f'  {"Composite Fitness":<32}  {sol["fitness"]:>12.6f}',
                f'  {"CL (Lift Coefficient)":<32}  {sol["CL"]:>12.4f}',
                f'  {"CD (Drag Coefficient)":<32}  {sol["CD"]:>12.4f}',
                f'  {"XCP (Centre of Press)":<32}  {sol["XCP"]:>12.4f}',
                f'  {"CL/CD (Lift-to-Drag)":<32}  {sol["CLCD"]:>12.4f}',
                '  ' + '-' * 56,
                '  18 PARAMETER VALUES:',
            ]
            for pn, pv in sol['params'].items():
                lines.append(f'    {LABELS[pn]:<32}  {pv:>12.4f}')
            lines += ['  ' + '=' * 60, '']
        con_append('TOP5_OPT', '\n'.join(lines))
    else:
        con_append('TOP5_OPT',
...
def clear_opt():
    ...
    _opt_idx = 0
    window['OPT_PLT_LBL'].update('')
    # Clear the canvas by destroying any embedded widgets
    try:
...
while True:
    ...
    if event == 'Estimate':
        ...
        set_prog('PB_P', 'PP_P', 'PM_P', 15,
                 'Running prediction...')
        set_status('Running aerodynamic prediction...',
                   color=C_AMBER)
    ...
    # ── Tab 2: Optimization ───────────────────────────────
        ...
        con_append('OPT_LOG',
                   ...
                   + '-' * 58)
        set_status(f'Running Custom DE [{mh}]...',
                   color=C_AMBER)
        threading.Thread(
            target=_opt_worker,
